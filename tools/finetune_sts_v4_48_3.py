...
class StepLossCSVCallback(TrainerCallback):
    def __init__(self, output_dir):
        self.output_dir = output_dir
        self.csv_path = os.path.join(output_dir, "step_loss.csv")
        self.file = None
        self.writer = None
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        # Trainer 每个 logging step 都会带 loss
        if "loss" in logs:
            self.writer.writerow([
                state.global_step,
                state.epoch,
                logs["loss"],
                logs.get("learning_rate", None),
            ])
            self.file.flush()
...
def main():
    ...
    if model_args.audio_model_freeze:
        ...
        load_pretrained_model(
            ...
            path="models/SenseVoiceSmall/model.pt",
    ...
    trainer.add_callback(StepLossCSVCallback(training_args.output_dir))
    # Training
    if training_args.do_train:
        checkpoint = None
