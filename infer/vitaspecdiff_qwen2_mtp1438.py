"""
Refactored Qwen2 speculative-decoding + audio-parallel pipeline for MTP mode [1, 4, 3, 8, 4, 10].

Key behavior:
- Keep first-chunk text slicing policy unchanged (same two conditions as legacy).
- Apply tqa4-style 4-text-token acceptance logic to every subsequent chunk.
- Use verified text (1+3) as acceptance anchor, then consume 4+10 group for audio continuation.
- When text has ended but audio is still being completed, directly accept audio tokens.
"""
