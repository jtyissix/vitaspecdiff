...
class VitaStreaming():
    ...
    def run_infer_stream(self,audio_tensor,output_dir):
        ...
        if vocoder_prediffuse_task_1 is None:
            ...
            this_step=10-min(8,self.audio_tokenizer.audio_decoder.flow.now_step)
...
if __name__ == "__main__":
    ...
    time_json_path = "/home/fit/renjujty/WORK/jty/vita/json/b1_8.json"
    # 只保存每个音频第一次生成的文本（jsonl，一行一个json），不存在会自动创建
    text_jsonl_path = "/home/fit/renjujty/WORK/vita_temp/sensitivity/b1/8/generated_text.jsonl"
    audio_save_path= "/home/fit/renjujty/WORK/vita_temp/sensitivity/b1/8/"
    data_csv_path = None
    ...
    pre_len=len(num_dict.get("b1_8_time", []))
    total_time_list = num_dict.get("b1_8_time", [])
    avg_time_list = num_dict.get("b1_8_avg_time", [])
    # ====== evaluation loop ======
    for i in range(pre_len, len(audio_path_list)):
        ...
        num_dict["b1_8_time"] = total_time_list
        num_dict["b1_8_avg_time"] = avg_time_list
