...
class VitaStreaming():
    ...
    def run_infer_stream(self,audio_tensor,output_dir):
        ...
        for tok,logit in self.draft_model.stream_generate(input_ids, max_new_tokens=8,
                                      steps_done=0,
                                      do_sample=False,
                                      eos_token_id=[151645, 151643],
        ...
        verify_logit=self.target_model.speculative_verify(
                    input_ids=input_ids,
                    draft_tokens=torch.cat(draft_toks,dim=-1).unsqueeze(0)
                )
        #breakpoint()
        draft_text_logit=torch.cat(draft_logits)
        ...
        draft_text_tokens=torch.cat(draft_toks)
...
if __name__ == "__main__":
    ...
    vita=VitaStreaming()
    with open('/home/fit/renjujty/WORK/jty/vita/json/specdiff_num_dict_tqa.json','r') as f:
    ...
    #num_list=[]
    for i in range(1000):
        response,num = vita.run_infer_stream(audio_path_list[i],'/home/fit/renjujty/WORK/vita_temp/')
        #time.sleep(1)
        if response is False:
            #fail_list.append(i)
            num_dict[num].append(i)
    #print("fail list:",fail_list,"lengeth:",len(fail_list))
    print("num dict:",num_dict)
    for k,v in num_dict.items():
        print(f"num {k}: {v}, length: {len(v)}")
    
    with open('/home/fit/renjujty/WORK/jty/vita/json/specdiff_num_dict_tqa.json','w') as f:
