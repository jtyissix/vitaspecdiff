...
class VitaStreaming():
    ...
    def run_infer_stream(self,audio_tensor,output_dir):
        ...
        for new_text in self.streamer:
            ...
            if "<|end_of_audio|>" == new_text:
                ...
                if num_audio_chunk == 0:
                    ...
                    vocoder_time = first_audio_time - llm_time
        ...
        return {"text":remove_audio_tokens(generated_text), 'audio': f"{output_dir}/{base_name}"},llm_time, vocoder_time
...
if __name__ == "__main__":
    ...
    time_json_path = "/home/fit/renjujty/WORK/jty/vita/json/aux.json"
    # 只保存每个音频第一次生成的文本（jsonl，一行一个json），不存在会自动创建
    text_jsonl_path = "/home/fit/renjujty/WORK/vita_temp/llama_auxtime_audio/generated_text.jsonl"
    audio_save_path= "/home/fit/renjujty/WORK/vita_temp/llama_auxtime_audio/"
    ...
    for i in range(len(audio_path_list)):
        ...
        for j in range(10):
            ...
            response, llm_time, vocoder_time = vita.run_infer_stream(wav_path,audio_save_path)
            time_list.append(llm_time + vocoder_time)
            llm_list.append(llm_time)
            vocoder_list.append(vocoder_time)
        ...
        # 取10次平均
        id=time_list.index(max(time_list))
        llm_mx = llm_list[id]
        vocoder_mx = vocoder_list[id]
        llm_time_list.append(llm_mx)
        vocoder_time_list.append(vocoder_mx)
        #print(f"avg time is {mx}")

        # 每个音频都更新一次时间json（保持你原逻辑）
        num_dict["llm_time"] = llm_time_list
        num_dict["vocoder_time"] = vocoder_time_list
